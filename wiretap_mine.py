# WTC-Lightcode channel with CCE loss, MINE estimation
...
def estimate_mi(secrecy_seed, model, train_mean, train_std, args, logging):
    ...
    for eve_snr in eve_list:
        ...
        std1_eve = 10 ** (-eve_snr * 1.0 / 10 / 2) #forward snr
        ...
        if hasattr(mi_estimator, 'avg_test_mi'):
            print(f"secrecy_seed{s}, bob snr: {args.snr1_bob}, eve snr: {eve_snr}, Eve Estimated Mutual Information: {mi_estimator.avg_test_mi} nats, {mi_estimator.avg_test_mi / math.log(2)} bits")
        else:
...
if __name__ == '__main__':
    ...
    secrecy_seed = [1, 1, 0, 1]
    print("seed", secrecy_seed)
    s_tensor, secreVec_1_decimal, secreVec_1_binary = secrecy_encode(args.q, combedVec_1, secrecy_seed, exp_table, log_table, args.device)
    ...
    estimate_mi(secrecy_seed, model, train_mean, train_std, args, logging)
